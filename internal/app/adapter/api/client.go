...
func (r *Router) clients(w http.ResponseWriter, req *http.Request) {
	// Change
	nameEntity, nameEntities := "client", "clients"
	...
	case http.MethodGet:
		if id == 0 {
			// Change
			entities, err := r.useCase.Clients(req.Context(), token)
			if err != nil {
			...
			// Change
			entity, err := r.useCase.Client(req.Context(), token, id)
			if err != nil {
				status := http.StatusInternalServerError
	...
	case http.MethodPost:
		...
		// Change
		id, err := r.useCase.CreateClient(req.Context(), token, &input)
		if err != nil {
			status := http.StatusInternalServerError
			if errors.Is(err, usecase.ErrUnauthorized) {
				status = http.StatusUnauthorized
	...
	case http.MethodPut:
		...
		// Change
		err := r.useCase.UpdateClient(req.Context(), token, id, &input)
		if err != nil {
			status := http.StatusInternalServerError
			if errors.Is(err, usecase.ErrUnauthorized) {
				status = http.StatusUnauthorized
	...
	case http.MethodDelete:
		...
		// Change
		err := r.useCase.DeleteClient(req.Context(), token, id)
		if err != nil {
			status := http.StatusInternalServerError
			if errors.Is(err, usecase.ErrUnauthorized) {
				status = http.StatusUnauthorized
