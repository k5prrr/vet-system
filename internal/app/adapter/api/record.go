...
func (r *Router) records(w http.ResponseWriter, req *http.Request) {
	// Change
	nameEntity, nameEntities := "record", "records"
	...
	case http.MethodGet:
		if id == 0 {
			// Change
			entities, err := r.useCase.Records(req.Context(), token)
			if err != nil {
			...
			// Change
			entity, err := r.useCase.Record(req.Context(), token, id)
			if err != nil {
				status := http.StatusInternalServerError
	...
	case http.MethodPost:
		...
		// Change
		id, err := r.useCase.CreateRecord(req.Context(), token, &input)
		if err != nil {
			status := http.StatusInternalServerError
			if errors.Is(err, usecase.ErrUnauthorized) {
				status = http.StatusUnauthorized
	...
	case http.MethodPut:
		...
		// Change
		err := r.useCase.UpdateRecord(req.Context(), token, id, &input)
		if err != nil {
			status := http.StatusInternalServerError
			if errors.Is(err, usecase.ErrUnauthorized) {
				status = http.StatusUnauthorized
	...
	case http.MethodDelete:
		...
		// Change
		err := r.useCase.DeleteRecord(req.Context(), token, id)
		if err != nil {
			status := http.StatusInternalServerError
			if errors.Is(err, usecase.ErrUnauthorized) {
				status = http.StatusUnauthorized
